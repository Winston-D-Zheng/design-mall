...
public class TaobaoOrderUploadServiceImpl implements TaobaoOrderUploadService {
    ...
    public void upload(MultipartFile file, Long shopId) {
        ...
        // 解析excel
        ArrayList<DbTbomsTaobaoOrder> entities = parseExcel(file);

        // 保存到数据库
        for (DbTbomsTaobaoOrder entity : entities) {
            dbTbomsTaobaoOrderService.saveOrUpdateByShopIdAndOrderNo(entity);
        }
    }
     ...
     * 解析excel
     */
    private ArrayList<DbTbomsTaobaoOrder> parseExcel(MultipartFile file) {
        ArrayList<DbTbomsTaobaoOrder> rt = new ArrayList<>();
        ...
        try (InputStream inputStream = file.getInputStream(); Workbook workbook = WorkbookFactory.create(inputStream)) {
            ...
            for (var row : sheet) {
                if (row.getRowNum() == 0) {
                    ...
                    try {
                        DbTbomsTaobaoOrder entity = new DbTbomsTaobaoOrder();
                        // 填充信息
     ...
     * 填充数据
     */
    private void fillEntity(DbTbomsTaobaoOrder entity, Row row) {
        ...
        BigDecimal buyerActualPaymentAmount = BigDecimal.valueOf(getCellValue(row, parseFailed, "买家实际支付金额"));
        LocalDateTime refundTime = getCellValue(row, parseFailed, "退款时间");
        BigDecimal refundAmount = BigDecimal.valueOf(getCellValue(row, parseFailed, "退款金额"));
        ...
        entity.setTaobaoShopName(shopName);
        entity.setTaobaoOrderNo(orderNo);
        entity.setTaobaoPayTime(payTime);
        entity.setTaobaoBuyerActualPaymentAmount(buyerActualPaymentAmount);
        entity.setTaobaoRefundTime(refundTime);
        entity.setTaobaoRefundAmount(refundAmount);
        entity.setTaobaoOrderStatus(orderStatus);
        entity.setTaobaoMerchantNotes(merchantNotes);
